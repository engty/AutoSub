...
直接返回JSON，不要包含其他内容。`;
  ...
  private generateDetails(report: TestReport): string {
    ...
    if (report.loginDetected) {
      lines.push(`✅ 登录成功（检测方式: ${report.loginMethod}, 耗时: ${report.loginDuration}ms）`);
    } else {
      lines.push('❌ 登录检测失败');
    }
    lines.push('');
    ...
    // 凭证捕获
    lines.push('### 2. 凭证捕获');
    lines.push(`- Cookie: ${report.credentials.cookies.found ? `✅ ${report.credentials.cookies.count}个` : '❌ 未找到'}`);
    lines.push(`- localStorage: ${report.credentials.localStorage.found ? `✅ ${report.credentials.localStorage.count}个键` : '❌ 未找到'}`);
    lines.push(`- sessionStorage: ${report.credentials.sessionStorage.found ? `✅ ${report.credentials.sessionStorage.count}个键` : '❌ 未找到'}`);
    lines.push('');

    // API检测
    lines.push('### 3. API检测');
    if (report.apiDetected && report.apiConfig) {
      lines.push(`✅ 检测到订阅API`);
      lines.push(`- API地址: ${report.apiConfig.url}`);
      lines.push(`- 请求方法: ${report.apiConfig.method}`);
      lines.push(`- 认证方式: ${report.apiConfig.authSource}`);
      if (report.apiConfig.authField) {
        lines.push(`- 认证字段: ${report.apiConfig.authField}`);
      }
      if (report.apiConfig.tokenField) {
        lines.push(`- Token字段: ${report.apiConfig.tokenField}`);
      }
    } else {
      lines.push('❌ 未检测到订阅API');
    }
    lines.push('');

    // 订阅验证
    if (report.subscriptionExtracted) {
      lines.push('### 4. 订阅验证');
      if (report.subscriptionValid) {
        lines.push(`✅ 订阅地址有效`);
        if (report.subscriptionValidation?.nodeCount) {
          lines.push(`- 节点数量: ${report.subscriptionValidation.nodeCount}个`);
        }
      } else {
        lines.push(`❌ 订阅验证失败: ${report.subscriptionValidation?.errorMessage}`);
      }
      lines.push('');
    }
    ...
    if (report.compatibility.canUseHttpApi) {
      lines.push('✅ **HTTP API 模式** - 支持静默后台更新（< 1秒）');
    } else {
      lines.push('❌ **HTTP API 模式** - 不支持');
    }

    if (report.compatibility.canUseBrowserMode) {
      lines.push('✅ **浏览器模式** - 支持自动登录和订阅提取');
    } else {
      lines.push('❌ **浏览器模式** - 不支持');
    }
    lines.push('');
    ...
    if (report.steps.length > 0) {
      ...
      report.steps.forEach((step, index) => {
        ...
        lines.push(`${index + 1}. ${icon} ${step.name} ${step.duration ? `(${step.duration}ms)` : ''}`);
        if (step.message) {
