...
**智能API检测与静默更新**
- 首次添加站点时自动检测订阅API的端点、认证方式和数据结构
- 自动保存API配置到本地，后续更新使用纯HTTP请求（无需浏览器）
- 三级提取策略：HTTP API（优先） → 缓存地址 → 浏览器方式（最后手段）
- 支持多种认证方式：Cookie、localStorage Token、混合认证
- **性能提升**: 更新速度从 5-10秒 降至 < 1秒
- **资源节省**: 内存占用从几百MB降至几KB
- **用户体验**: 完全后台静默执行，不再弹出浏览器窗口
